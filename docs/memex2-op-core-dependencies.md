...
2. **[core] Markdown handler** — a `handlers/markdown/handler.js` alongside the HTML handler that parses YAML frontmatter + body `[[wikilinks]]`. Resolution is **declared-URI, per-handler** (reworked in #246, superseding the original whole-instance pass): each document declares its own identity via a frontmatter `uriField` (default `uri`), which the handler uses to set `@id` directly; the caller builds a `name -> declared uri` lookup once with the exported `buildTargetMap(entries, { uriField })` (path-qualifier collision handling via slash-containing path tails; a colliding bare key is marked with the exported `AMBIGUOUS` sentinel) and passes it as `options.wikilinkTargets`. The handler resolves each extracted wikilink against that lookup inline, emitting `{ type:'link', uri }` edges directly on `output.octothorpes` (deduped, no self-edges); no-match/ambiguous links never become edges, only a `{ target, reason }` entry on `output.warnings`. There is no separate resolve/apply pass and no `resolvedLinks`/`unresolvedLinks` report object anymore. Input contract: `(rawMarkdown, _, { mode:'markdown', uriField, wikilinkTargets })`. **Hard blocker** for the curate-in-Obsidian workflow. → **octothorp.es #238/#246**; plan: `octothorp.es/docs/plans/point7/2026-07-07-markdown-handler-wikilinks.md`.
